if (FALSE) {
  ...
  # simulate data
  data = simData(n = 3e6, pars = pars, tt = tt, pdfND = NULL, normalizePdfs = FALSE, return.pdf = TRUE,
...
if (FALSE) {
  ...
  idx <- seq(1, length(tt), length.out = length(tt.est))
  res3 <- estDstarM(data = data$dat, tt = tt.est, restr = restr.ter, fixed = fixed, DstarM = FALSE,
